...
const PostElement = ({
   ...
   * If set to false, the heart icon will be hollow and clicking it will call the 'like_post' lambda function
   */
  const [isPosterFollowed, setIsPosterFollowed] = useState(
    isPosterFollowedParam
  );

  const [numberOfLikes, setNumberOfLikes] = useState(
    Number(postObject?.numberOfLikes)
  ); // the number of likes on the post
  const [numberOfComments, setNumberOfComments] = useState(
    Number(postObject?.numberOfComments)
      ...
      <div className="PE-comment-big-box" key={comment?.commentID}>
        <div className="PE-comment-box">
          <div className="PE-comment-username">
            {comment?.userNameOfCommenter}:
          </div>
          <div className="PE-comment-text">{comment?.content}</div>
        </div>
        {/* 
                    // 6
                    //
  ...
  return (
    <div
      className={
        isGridLayout === true ? "post-square-container" : "post-container"
      }
    >
      ...
      <span className="post-user-email">Posted by {postObject?.userName}</span>
      {isDetailsVisible && (
      ...
      <div className="post-like-comment-container">
        <div className="post-like-container" onClick={() => handleLikes()}>
          {isPostLiked === true ? (
            <svg
              className="post-filled-like-icon"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              fill="currentColor"
            >
            ...
            <svg
              className="post-hollow-like-icon"
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth="1.5"
